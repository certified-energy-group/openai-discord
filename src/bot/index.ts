...
export class Bot implements Runnable {
  ...
  constructor(ai: AI) {
     ...
     */
    this.loadAndInitializeCommandsFromFolder(`${__dirname}/commands`)
      .then((commands) => {
  ...
  private async loadAndInitializeCommandsFromFolder(folderPath: string): Promise<Command[]> {
    ...
    const tsFiles = files.filter(
      (file) =>
        file.endsWith("js") || // For compiled files on production
        file.endsWith(".ts") ||
        file.endsWith(".tsx"),
    ); // For source files on development
  ...
  private async handleSlashCommand(interaction: CommandInteraction): Promise<void> {
     ...
     */
    const slashCommand = this._commands.find((command) => command.name === interaction.commandName);
    if (!slashCommand) {
  ...
  run(): void {
    ...
    this._client.on("ready", async () => {
      ...
      try {
        ...
        await rest.put(Routes.applicationCommands(this._client.application.id), { body: availableCommands });

        this._logger.logService.info(
          `Discord API REST [${availableCommands.length}] commands registered successfully.`,
        );
      } catch (error) {
