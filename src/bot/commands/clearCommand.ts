...
export class ClearCommand extends Command {
  ...
  protected async execute(client: Client, interaction: CommandInteraction): Promise<void> {
     ...
     */
    const channelMessages = (await this.fetchMessages(amount)).filter(
      (message) => message.interaction?.user.id === interaction.user.id && message.embeds.length > 0,
    );
    ...
    if (channelMessages.size > 0) {
      if (interaction.channel?.type === ChannelType.GuildText) {
         ...
         */
        for (const [_message, message_object] of channelMessages) {
